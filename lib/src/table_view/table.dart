...
class RenderTableViewport extends RenderTwoDimensionalViewport {
  ...
  double get _pinnedColumnsExtent => _lastPinnedColumn != null ? _columnMetrics[_lastPinnedColumn]!.trailingOffset : 0.0;
  ...
  void _updateAllMetrics() {
    ...
    for (int column = 0; column < delegate.columnCount; column++) {
      ...
      final TableSpan configuration = needsDelegateRebuild ? delegate.buildColumn(column) : span!.configuration;
      span ??= _Span();
      span.update(
        isPinned: isPinned,
      ...
      if (!isPinned) {
        ...
        final double targetColumnPixel = cacheExtent + horizontalOffset.pixels + viewportDimension.width - startOfPinnedColumn;
        if (span.trailingOffset >= targetColumnPixel && _lastNonPinnedColumn == null) {
    ...
    for (int row = 0; row < delegate.rowCount; row++) {
      ...
      assert(needsDelegateRebuild || span != null);
      final TableSpan configuration = needsDelegateRebuild ? delegate.buildRow(row) : span!.configuration;
      span ??= _Span();
      span.update(
        isPinned: isPinned,
        configuration: configuration,
      ...
      if (!isPinned) {
        ...
        final double targetRowPixel = cacheExtent + verticalOffset.pixels + viewportDimension.height - startOfPinnedRow;
        if (span.trailingOffset >= targetRowPixel && _lastNonPinnedRow == null) {
    ...
    if (_rowMetrics.length <= delegate.pinnedRowCount) {
      ...
      maxVerticalScrollExtent = math.max(
        0.0,
        _rowMetrics[lastRow]!.trailingOffset - viewportDimension.height + startOfPinnedRow,
      );
    }
    ...
    if (_columnMetrics.length <= delegate.pinnedColumnCount) {
      ...
      maxHorizontalScrollExtent = math.max(
        0.0,
        _columnMetrics[lastColumn]!.trailingOffset - viewportDimension.width + startOfPinnedColumn,
      );
    }
  ...
  void _updateFirstAndLastVisibleCell() {
    _firstNonPinnedColumn = null;
    _lastNonPinnedColumn = null;
    final double targetColumnPixel = cacheExtent + horizontalOffset.pixels + viewportDimension.width - _pinnedColumnsExtent;
    ...
    _firstNonPinnedRow = null;
    _lastNonPinnedRow = null;
    final double targetRowPixel = cacheExtent + verticalOffset.pixels + viewportDimension.height - _pinnedRowsExtent;
        ...
        if (cell != null) {
          ...
          if (cellParentData.rowMergeStart != null || cellParentData.columnMergeStart != null) {
            ...
            final int lastColumn =
                cellParentData.columnMergeStart == null ? column : firstColumn + cellParentData.columnMergeSpan! - 1;
            assert(_debugCheckMergeBounds(
            ...
            mergedRowOffset =
                baseRowOffset + _rowMetrics[firstRow]!.leadingOffset + _rowMetrics[firstRow]!.configuration.padding.leading;
  ...
  void _paintCells({
    ...
    for (int column = leadingVicinity.column; column <= trailingVicinity.column; column++) {
      ...
      if (columnSpan.backgroundDecoration != null || columnSpan.foregroundDecoration != null || _mergedColumns.contains(column)) {
        final List<({RenderBox leading, RenderBox trailing})> decorationCells = <({RenderBox leading, RenderBox trailing})>[];
        ...
        for (final ({RenderBox leading, RenderBox trailing}) cell in decorationCells) {
          ...
          final int columnIndex = parentDataOf(cell.leading).columnMergeStart ?? parentDataOf(cell.leading).tableVicinity.column;
    ...
    for (int row = leadingVicinity.row; row <= trailingVicinity.row; row++) {
      ...
      if (rowSpan.backgroundDecoration != null || rowSpan.foregroundDecoration != null || _mergedRows.contains(row)) {
        ...
        for (final ({RenderBox leading, RenderBox trailing}) cell in decorationCells) {
          ...
          final int rowIndex = parentDataOf(cell.leading).rowMergeStart ?? parentDataOf(cell.trailing).tableVicinity.row;
